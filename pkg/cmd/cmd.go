...
func init() {
	Command = &cli.Command{
		Name:      "init-omega-py",
		Usage:     "CLI for the init_omega.py API",
		Suggest:   true,
		Version:   Version,
		...
		Commands: []*cli.Command{
			...
			},
			{
				...
				UsageText:       "init-omega-py @manpages [-o init-omega-py.1] [--gzip]",
				Hidden:          true,
...
func generateManpages(ctx context.Context, c *cli.Command) error {
	...
	if c.Bool("text") {
		file, err := os.Create(filepath.Join(dir, "man1", "init-omega-py.1"))
		if err != nil {
			return err
		}
		defer file.Close()
	...
	if c.Bool("gzip") {
		file, err := os.Create(filepath.Join(dir, "man1", "init-omega-py.1.gz"))
		if err != nil {
			return err
		}
		defer file.Close()
